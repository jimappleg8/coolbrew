...
<?php endif; ?>
   ...
   <h1>Contact Us: Messages <span>(<?=$message_count;?>) | <a href="<?=site_url('sites/contactus_spam/index/'.$site_id.'/'.$offset);?>">Spam</a> (<?=$spam_count;?>)</span></h1>
...
<form method="post" action="<?=site_url('sites/contactus/set_filter/'.$offset);?>">
...
<a style="text-decoration:none; <?php if ($message['status'] == 'unread'): ?> font-weight:bold;<?php endif; ?>" href="<?=site_url('sites/contactus/detail/'.$site_id.'/'.$message['id'].'/'.$offset.'/'.$last_action);?>"<?php if ($message['status'] == 'unread'): ?> style="font-weight:bold;"<?php endif; ?>><?=character_limiter(strip_tags($message['comment']), 60);?></a>
