...
class Scaffolding {
   ...
   function Scaffolding($db_table)
   {
      ...
      {
         $this->current_table = $db_table['db_table'];
      }
      else
      {
      ...
      $this->CI->load->_ci_view_path = BASEPATH.'views/scaffolding/';
      ...
      // Set a few globals
      $data = array(
                  'image_url'   => $this->CI->config->system_url().'views/scaffolding/images/',
   ...
   /**
    * Generates the specified method for the table
    *
    * @access   public
    * @return   string   the add method
    */
   function get_method($method)
   {
      $data['table_name'] = $this->current_table;
      $data['fields'] = $this->CI->db->field_data($this->current_table);
      return $this->CI->load->view($method.'_method', $data, TRUE);
   }

   // --------------------------------------------------------------------
   
   /**
    * Generates the specified  view for the table
    *
    * @access   public
    * @return   string   the add view
    */
   function get_view($view)
   {
      $data['table_name'] = $this->current_table;
      $data['fields'] = $this->CI->db->field_data($this->current_table);
      return $this->CI->load->view($view.'_view', $data, TRUE);
   }
   ...
   function add()
   {
      ...
      $db_fields = $this->CI->db->field_data($this->current_table);
      
      foreach ($db_fields AS $db_field)
      {
         if ($db_field->primary_key == 1) continue;
         $rules[$db_field->name] = 'trim';
      }
      ...
      {
         if ($db_field->primary_key == 1) continue;
         $fields[$db_field->name] = $db_field->name;
      }
      ...
      {
         if ($db_field->primary_key == 1) continue;
         $defaults[$db_field->name] = $db_field->default;
      }
      $this->CI->validation->set_defaults($defaults);

      $this->CI->validation->set_error_delimiters('<div style="color:red;">', '</div>');
      
      if ($this->CI->validation->run() == FALSE)
      {
         $data['title'] = ( ! isset($this->lang['scaff_add'])) ? 'Add Data' : $this->lang['scaff_add'];
         $data['fields'] = $db_fields;
         $data['action'] = $this->base_uri.'/add';

         $this->CI->load->vars($data);
         echo $this->CI->load->view('add', NULL, TRUE);
      }
      else
      {
         $this->insert();
   ...
   function insert()
   {      
      $fields = $this->CI->validation->_fields;
      
      foreach ($fields AS $key => $value)
         $values[$key] = $this->CI->input->post($key);
      
      if ($this->CI->db->insert($this->current_table, $values) === FALSE)
      {
         $this->add();
      }
      else
   ...
   function view()
   {
      ...
      $data['title'] = ( ! isset($this->lang['scaff_view'])) ? 'View Data' : $this->lang['scaff_view'];
      $data['query'] = $query;
      $data['fields'] = $fields;
      $data['primary'] = $primary;
      $data['paginate'] = $this->CI->pagination->create_links();
   ...
   function edit()
   {
      ...
      {
         if ($db_field->primary_key == 1) continue;
         $defaults[$db_field->name] = $field_data->{$db_field->name};
      }
      $this->CI->validation->set_defaults($defaults);
      ...
      if ($this->CI->validation->run() == FALSE)
      {
         $data['title'] = ( ! isset($this->lang['scaff_edit'])) ? 'Edit Data' : $this->lang['scaff_edit'];
         $data['fields'] = $db_fields;
         $data['action'] = $this->base_uri.'/edit/'.$id;

         $this->CI->load->vars($data);
         echo $this->CI->load->view('edit', NULL, TRUE);
      }
      else
      {
         $this->update($id);
   ...
   function delete()
   {
      ...
      $data['title'] = ( ! isset($this->lang['scaff_delete'])) ? 'Delete Data' : $this->lang['scaff_delete'];
      $data['message'] = $message;
      $data['no'] = anchor(array($this->base_uri, 'view'), ( ! isset($this->lang['scaff_no'])) ? 'No' : $this->lang['scaff_no']);
      $data['yes'] = anchor(array($this->base_uri, 'do_delete', $this->CI->uri->segment(4)), ( ! isset($this->lang['scaff_yes'])) ? 'Yes' : $this->lang['scaff_yes']);
