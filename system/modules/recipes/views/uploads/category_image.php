...
<?=form_open_multipart('uploads/upload_category_image/'.$site_id.'/'.$category_id);?>
...
<?php
if ($category['ImageWidth'] >= $category['ImageHeight'])
{
...
<img src="<?=$base_url;?><?=$category['ImageFile'];?>" <?=$width_height;?> alt="<?=$category['CategoryName'];?>" />
</div>

<form>
<input type="button" onclick="document.location='<?=site_url('uploads/upload_category_image/'.$site_id.'/'.$category_id.'/1');?>'" value="Remove" style="margin-top:12px;" />
</form>
