...
function dodelete()
{
  ...
  {
    document.location = "<?=site_url('sites/vendors/delete/'.$site_id.'/'.$vendor_id);?>";
  }
}
...
<div id="Wrapper">
   ...
   <a class="admin" href="#" onclick="dodelete()">Delete</a> | <a class="admin" href="<?=site_url('sites/vendors/index/'.$site_id);?>">Cancel</a>

               </div>
               
   <h1>Edit this Site Vendor</h1>
...
<h1 style="margin-bottom:0;"><?=$vendor['VendorName'];?></h1>
<p style="font-size:1.3em;"><?=$site_vendor['ServiceName'];?></p>
</div>

               <div id="basic-form">

<form method="post" action="<?=site_url('sites/vendors/edit/'.$site_id.'/'.$vendor_id.'/'.$last_action);?>">

<p class="blockintro">You can change what vendor is associated with this site:</p>
<div class="block">
   <dl>
      <dt class="required"><label for="Vendor">Vendor:</label></dt>
      ...
      <dt class="required"><label for="Status">Status:</label></dt>
...
<div class="action">
   <?=form_submit(array('name'=>'submit', 'class'=>'submit', 'value'=>'Save changes'))?> or <a class="admin" href="<?=site_url('sites/vendors/index/'.$site_id);?>">Cancel</a>
</div>
