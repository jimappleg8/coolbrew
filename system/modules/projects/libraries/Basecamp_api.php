...
class Basecamp_api {
   ...
   /**
    * This will return an alphabetical list of all message categories
   ...
   /**
    * This will return an alphabetical list of all file categories in 
   ...
   /**
    * Marks the specified todo list item as "complete". If the item is 
    * already completed, this does nothing. 
    * 
    */
   ...
   /**
    * Returns the metadata for all of the todo lists in a given project.
    *
    ...
    * You can either return all milestones, or only those that are late,
    ...
    * @param   bool      move upcoming milestones?
    * @param   bool      move upcoming milestones off weekends?
    * @return  array     request results
    */
   function update_milestone($id, $ms, $move = true, $weekends = true)
   {
      $request['milestone'] = $ms;
      $request['move-upcoming-milestones'] = $move;
      $request['move-upcoming-milestones-off-weekends'] = $weekends;

      return $this->hook("/milestones/update/{$id}", "milestone", array("milestone" => $ms));
   }
   ...
   function request($url, $params = false)
   {
      if ( ! $this->use_curl)
      {
         ...
         if ($this->proxy != "")
         {
            $req->setProxy($this->proxy, $this->proxy_port);
         }
      ...
      else
      {
         ...
         if ($this->proxy != "")
         {
            curl_setopt($ch, CURLOPT_PROXY, $this->proxy.':'.$this->proxy_port);
         }
         ...
         // if we're sending stuff, serialize the data
         if ($params)
         {
            $xml = $this->serialize($params);
            curl_setopt($ch, CURLOPT_POST, true);
            ($xml) ? curl_setopt($ch, CURLOPT_POSTFIELDS, $xml) : false;
         }
