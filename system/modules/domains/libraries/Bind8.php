...
class Bind8 {
	...
	var $NAMEDCONF = "/etc/named.conf";	// Location of named.conf
	var $DIRECTORY = "/etc/named";		// Location of dns flies, err.. files.
...
//	Reads in the domain files... *sigh*
	...
	if(!empty($filename)){
		...
		$Mytemp = explode("\n",$contents);
		list($origin,$bufa) = explode(" ",$Mytemp[0],2);
		list($bufb,$crap,$crap,$nameserver,$contact) = explode('[[:space:]]+',$Mytemp[1],6);
...
if($this->DEBUG){
		...
		list($crap,$serial,$refresh,$retry,$expire,$ttl) = explode('[[:space:]]+',$Mytemp[2],7);
...
//			if(empty($Mytemp[$count]))                                      { break; }
		if($loop==0){	
			list($bufc,$bufa,$bufb,$crap) = explode('[[:space:]]+',$Mytemp[$count],4);
...
if($this->DEBUG){ echo "Found MX record!<BR>"; }
			...
			if($bufc=="\$ORIGIN"){ 
					if($this->DEBUG){ echo "Found ORIGIN record!<BR>"; }
					$loop=1; $count++;
			 }
		}
		...
		if($loop==1){
			...
			list($bufa,$bufb,$bufc,$crap) = explode('[[:space:]]+',$Mytemp[$count],4);

if($this->DEBUG){ echo "<font color=green>In loop1: [$bufa][$bufb][$bufc][$crap]</font>\n<BR>"; }

			if($bufc=="CNAME"){

				if($this->DEBUG){ echo "Found CNAME record!<BR>"; }

//				$Myjunk["$domainame"]["CNAME"][$ctrack]="$bufa" . ":$crap";
...
if($this->DEBUG){
	echo "<font color=red>We are writing $domainfile</font>\n<BR>";
		...
		list($dhead,$dtail) = explode("\.",$domain,2);
...
if($this->DEBUG){
		...
		if(!empty($echeck)){
			for($count=0;$count<count($this->DOMAINS[$domain]["NS"]);$count++){
				...
				if(!empty($NS)) fwrite($fd,"\t\tIN\tNS\t$NS\n");
			}
		}
		...
		if(!empty($echeck)){
			for($count=0;$count<count($this->DOMAINS[$domain]["MX"]);$count++){
                           $MX = $this->DOMAINS[$domain]["MX"][$count];
                                if(!empty($MX)) fwrite($fd,"\t\tIN\tMX\t$MX\n");
                        }
                }
		...
		if(!empty($echeck)){
				for($count=0;$count<count($this->DOMAINS[$domain]["A"]);$count++){
					if(eregi(":",$this->DOMAINS[$domain]["A"][$count])){
						list($dhead,$A) = explode(":",$this->DOMAINS[$domain]["A"][$count],2);
						if(!empty($A)) fwrite($fd,"$dhead\t\tIN\tA\t$A\n");
					}
				}
		}
...
/*  //Mah boy, cnames can't point to wierd things....
		...
		if(!empty($echeck)){
                                for($count=0;$count<count($this->DOMAINS[$domain]["CNAME"]);$count++){
                                        if(eregi(":",$this->DOMAINS[$domain]["CNAME"][$count])){
                                                list($dhead,$CNAME) = explode(":",$this->DOMAINS[$domain]["CNAME"][$count],2);
...
if($this->DEBUG){
	...
	function named()
	{
		...
		for ($count=0; $count<count($Mytemp); $count++)
		{
			...
			{
				list($crap, $buf) = explode('[[:space:]|{]+', $Mytemp[$count], 3);
				$crap = trim($buf);
				$buf = $crap;
...
/* // Let's keep it simple for now and just save this stuff. 
			...
			list($crap,$bufa,$bufb) = explode('[[:space:]]+',$Mytemp[$count],3);
...
// Now we "activate" the new domain......
		if($command=="add"){
			...
			$named[$this->DOMAIN]["zone"][$ztrack]="file \"" . $this->DOMAIN ."\";"; $ztrack++;
			$ztrack=0;
...
//      Sets the Contact as email addy.
		...
		if ( ! $email)
		{
			$this->error("Contact email address not specified.",0);
			return false;
		}
...
//      Note: strings must match!
		...
		$priority = 0;
                ...
                for($count=0;$count<count($this->DOMAINS[$domain]["MX"]);$count++){
                        ...
                        list($priority,$curmx)=explode(" ",$this->DOMAINS[$domain]["MX"][$count]);
