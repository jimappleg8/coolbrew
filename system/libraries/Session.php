 ...
 * @link		http://ciforge.com/projects/db-session
 */
class CI_Session {
	...
	function sess_run()
	{
		...
		if ($this->object->config->item('sess_use_database') === TRUE AND $this->object->config->item('sess_table_name') != '')
		{
			...
			$this->object->load->database('write');
		}
		else
		{
		 ...
		 *  Set the "now" time
		 *
 		 * It can either set to GMT or time(). The pref is set in the 
 		 * config file.  If the developer is doing any sort of time 
 		 * localization they might want to set the session time to 
 		 * GMT so they can offset the "last_activity" and "last_visit" 
 		 * times based on each user's locale.
		 *
		 */
	...
	{	
		...
		log_message('debug', $this->object->db->last_query());

		if ($query->num_rows() == 0)
		{
			log_message('debug', 'Session DB query did not find active session.');
			$this->sess_destroy();
			return FALSE;
		}
		else
	...
	{	
		...
		log_message('debug','sending session cookie: '.$this->sess_cookie.', '.$this->userdata['session_id'].', '.($this->sess_length + $this->now).', '.$this->object->config->item('cookie_path').', '.$this->object->config->item('cookie_domain').', 0');
		setcookie(
					$this->sess_cookie, 
		...
		// cookie info is updated only once a page load, so this fixes that
		$_COOKIE[$this->sess_cookie] = $this->userdata['session_id'];
	...
	/**
     * Set a flash data item which will be available only in next request 
     *
     * Following the next request, it will be automatically deleted from
     * the session array. You can use it to implement "Save succeeded" 
    ...
    /**
     * Keep existing flash data available to next request.
     *
     * @access	public
	 * @param	string
	 * @return	void
    ...
    /**
     * Return flash data for the given key.
     *
     * @access	public
	 * @param	string
	 * @return	mixed
    ...
    /**
     * Mark flash session attributes as 'old'
     *
     * @access	public
	 * @return	void
     */
    ...
    /**
     * Remove flash session marked as 'old'
     *
     * @access	private
	 * @return	void
     */
