...
//        }
        ...
        // This is to shape the method names the way I want them.

        $depth = $this->uri->total_segments();

		// The class method called is determined by the http method and the 
		// key words in the URI
		$object_called = '';
		for ($i=2; $i<=$depth; $i+=2)
		{
			$object_called .= $this->uri->segment($i).'_';
		}
		// If the URI ends with a variable, "_item" is appended to the class method
		if ($depth & 1)
		{
			// The number is odd
			$object_called .= 'item_';
        ...
        // end Jim Applegate's additions.
        // --------------------------------------------------------------------

        $controller_method = $object_called.$this->request->method;
...
//        global $CFG;
    ...
    protected function _detect_output_format()
    {
        ...
        // OR a format has been passed as an argument in a post and it is supported
        if (isset($this->_post_args['format']) and array_key_exists($this->_post_args['format'], $this->_supported_formats)) {
            return $this->_post_args['format'];
        }
     ...
     */
    public function get($key = null, $xss_clean = true, $no_result = FALSE)
    {
        if ($key === null) {
            return $this->_get_args;
        }

        return array_key_exists($key, $this->_get_args) ? $this->_xss_clean($this->_get_args[$key], $xss_clean) : $no_result;
    }
     ...
     */
    public function options($key = null, $xss_clean = true, $no_result = FALSE)
    {
        if ($key === null) {
            return $this->_options_args;
        }

        return array_key_exists($key, $this->_options_args) ? $this->_xss_clean($this->_options_args[$key], $xss_clean) : $no_result;
    }
     ...
     */
    public function head($key = null, $xss_clean = true, $no_result = FALSE)
    {
        if ($key === null) {
            return $this->head_args;
        }

        return array_key_exists($key, $this->head_args) ? $this->_xss_clean($this->head_args[$key], $xss_clean) : $no_result;
    }
     ...
     */
    public function post($key = null, $xss_clean = true, $no_result = FALSE)
    {
        if ($key === null) {
            return $this->_post_args;
        }

        return array_key_exists($key, $this->_post_args) ? $this->_xss_clean($this->_post_args[$key], $xss_clean) : $no_result;
    }
     ...
     */
    public function put($key = null, $xss_clean = true, $no_result = FALSE)
    {
        if ($key === null) {
            return $this->_put_args;
        }

        return array_key_exists($key, $this->_put_args) ? $this->_xss_clean($this->_put_args[$key], $xss_clean) : $no_result;
    }
     ...
     */
    public function delete($key = null, $xss_clean = true, $no_result = FALSE)
    {
        if ($key === null) {
            return $this->_delete_args;
        }

        return array_key_exists($key, $this->_delete_args) ? $this->_xss_clean($this->_delete_args[$key], $xss_clean) : $no_result;
    }
     ...
     */
    public function patch($key = null, $xss_clean = true, $no_result = FALSE)
    {
        if ($key === null) {
            return $this->_patch_args;
        }

        return array_key_exists($key, $this->_patch_args) ? $this->_xss_clean($this->_patch_args[$key], $xss_clean) : $no_result;
    }
