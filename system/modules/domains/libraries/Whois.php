...
require_once('phpwhois/whois.client.php');
require_once('phpwhois/whois.idna.php');
...
class Whois extends WhoisClient
	...
	function Whois()
		{
		// Load DATA array
		@require('phpwhois/whois.servers.php');
	...
	function Lookup($query = '', $is_utf = true)
		...
		if ($tld && $server)
			...
			foreach($tldtests as $htld)
				...
				if (($fp = @fopen('phpwhois/whois.'.$htld.'.php', 'r', 1)) and fclose($fp))
				    {
					$handler = $htld;
					break;
					}
