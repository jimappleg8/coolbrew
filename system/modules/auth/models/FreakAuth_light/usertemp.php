...
class UserTemp extends Model 
{
	...
	{	
		...
		return $this->db->get($this->_prefix.'user_temp');
	}
	...
	function insertUserForRegistration($values)
	{
	   /* foreach($values as $key=>$value)
        ...
        $this->db->insert($this->_prefix.'user_temp', $values);
	}
	...
	/**
	 * Finds the user that needs to be activated after he pressed
	...
	{
		$this->db->where('id', $id);
		$this->db->delete($this->_prefix.'user_temp'); 
	}
	...
	{	
		return$this->db->query('SELECT id AS id, UNIX_TIMESTAMP(created) AS created FROM '.$this->_prefix.'user_temp');
