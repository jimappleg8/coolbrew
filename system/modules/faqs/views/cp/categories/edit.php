...
function dodelete()
{
  ...
  {
    document.location = "<?=site_url('cp/categories/delete/'.$category_id.'/'.$last_action);?>";
  }
}
...
<form method="post" action="<?=site_url('cp/categories/edit/'.$site_id.'/'.$category_id.'/'.$last_action);?>">
...
<div class="action">
   <?=form_submit(array('name'=>'submit', 'class'=>'submit', 'value'=>'Save changes'))?> or <a class="admin" href="<?=site_url('cp/categories/index/'.$site_id);?>">Cancel</a>
</div>
