...
class CI_Validation {
	...
	// Addition, Begin - coolbrew: extension to the validation class as 
	// shown in http://www.codeigniter.com/forums/viewthread/47337/
	// modified to better match the functionality of Set Rules method
	
	/**
	 * Set Defaults
	 *
	 * This fuction takes an array of field names and default values as
	 * input and assigns the values to the fields.
	 *
	 * @access	public
	 * @param	array
	 * @return	void
	 */
	function set_defaults($data, $default = '')
	{
		if ( ! is_array($data))
		{
			if ($default == '')
		...
		{
			$this->$key = ( ! isset($_POST[$key])) ? $val : $this->$key;
		}
	}
	...
	{
		// Change, Begin - coolbrew: change to allow default to be set
		/*
		// Original
		if ($field == '' OR $value == '' OR  ! isset($_POST[$field]))
		{
		...
		// Original
		*/
		if ($field == '' OR $value == '' OR  ( ! isset($_POST[$field]) AND ! isset($this->$field)))
		{
			return '';
		}
