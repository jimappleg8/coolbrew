...
class Punbb extends Controller {

   // Here you can add additional smilies if you like (please note that you must escape singlequote and backslash)
   var $smiley_text = array(':)', '=)', ':|', '=|', ':(', '=(', ':D', '=D', ':o', ':O', ';)', ':/', ':P', ':lol:', ':mad:', ':rolleyes:', ':cool:');
   var $smiley_img = array('smile.png', 'smile.png', 'neutral.png', 'neutral.png', 'sad.png', 'sad.png', 'big_smile.png', 'big_smile.png', 'yikes.png', 'yikes.png', 'wink.png', 'hmm.png', 'tongue.png', 'lol.png', 'mad.png', 'roll.png', 'cool.png');
   
   // this will need to be pulled from user info...
   var $pun_user = array(
          'show_img' => '1',
          'show_img_sig' => '1',
          'show_smilies' => '1',
   ...
   /*
    * TAG: display X number of new entries.
    */
   function new_entries()
   {
      // (int) the total number of entries to display
      $show = $this->tag->param(1, 15);
      // (mixed) the forum ID(s) to include - can be a single int or an array
      $fid = $this->tag->param(2);
      // (mixed) the forum ID(s) to exclude - can be a single int or an array
      $nfid = $this->tag->param(3);
      
      // I'm hard-wiring this, but it will need to be via config file
      $prefix = "cbcms_";
      
      $order_by = 't.posted';
      $forum_sql = '';
      
      // Were any specific forum IDs supplied?
      if ($fid)
      {
         if ( ! is_array($fid))
         {
      ...
      $sql = 'SELECT t.id, t.poster, t.subject, t.posted, t.last_post, '.
             'f.id AS fid, f.forum_name, p.message '.
             'FROM '.$prefix.'topics AS t '.
             'INNER JOIN '.$prefix.'forums AS f '.
                'ON f.id=t.forum_id '.
             'INNER JOIN '.$prefix.'posts AS p '.
                'ON t.id=p.topic_id '.
             'LEFT JOIN '.$prefix.'forum_perms AS fp '.
                'ON (fp.forum_id=f.id AND fp.group_id=3) '.
             'WHERE (fp.read_forum IS NULL OR fp.read_forum=1) '.
             'AND t.moved_to IS NULL'.$forum_sql.' '.
             'ORDER BY '.$order_by.' DESC LIMIT '.$show;
      ...
      {
         $entries[$i]['message'] = $this->_preparse_bbcode($entries[$i]['message'], $errors);
         $entries[$i]['message'] = $this->_parse_message($entries[$i]['message'], '0');
      }
      
      $data['entries'] = $entries;
      
      $this->load->view('new_entries', $data);
   }
   
   // -----------------------------------------------------------------------
   
   /*
    * TAG: display X number of the newest top entries.
    */
   function top_entries()
   {
      ...
      $order_by = 't.id DESC, t.posted ASC';
      $forum_sql = '';
      ...
      for($i=0; $i<count($entries); $i++)
      {
         // limit the results to just the first entry for each topic
         if ($entries[$i]['id'] != $tid)
         {
            $tid = $entries[$i]['id'];
            $top_entries[$count] = $entries[$i];
            $top_entries[$count]['message'] = $this->_preparse_bbcode($top_entries[$count]['message'], $errors);
            $top_entries[$count]['message'] = $this->_parse_message($top_entries[$count]['message'], '0');
            $count++;
         }
      }
      
      $data['entries'] = $top_entries;
      
      $this->load->view('top_entries', $data);
   }
    ...
    */
   function _preparse_bbcode($text, &$errors, $is_signature = false)
   {
      ...
      // Do the more complex BBCodes 
      // (also strip excessive whitespace and useless quotes)
      $a = array( '#\[url=("|\'|)(.*?)\\1\]\s*#i',
               '#\[url\]\s*#i',
               '#\s*\[/url\]#i',
               '#\[email=("|\'|)(.*?)\\1\]\s*#i',
      ...
      if ( ! $is_signature)
      {
         $overflow = $this->_check_tag_order($text, $error);
...
//         message($lang_prof_reg['Signature quote/code']);
    ...
    */
   function _check_tag_order($text, &$error)
   {
      ...
      while (true)
      {
         ...
         // We are interested in the first quote 
         // (regardless of the type of quote)
         ...
         else if ($q_end < min($q_start, $c_start, $c_end))
         {
            if ($q_depth == 0)
            {
               $error = $this->lang_common['BBCode error'].' '.$this->lang_common['BBCode error 1'];
               return;
            }
         ...
         else if ($c_start < min($c_end, $q_start, $q_end))
         {
            ...
            {
               $error = $this->lang_common['BBCode error'].' '.$this->lang_common['BBCode error 2'];
               return;
            }
            else
            {
         ...
         {
            $error = $this->lang_common['BBCode error'].' '.$this->lang_common['BBCode error 3'];
            return;
         }
      }
      ...
      if ($q_depth)
      {
         $error = $this->lang_common['BBCode error'].' '.$this->lang_common['BBCode error 4'];
         return;
      }
      else if ($q_depth < 0)
      {
         $error = $this->lang_common['BBCode error'].' '.$this->lang_common['BBCode error 5'];
         return;
      }
   ...
   /*
    * Split text into chunks ($inside contains all text inside $start and 
    * $end, and $outside contains all text outside)
    */
   function _split_text($text, $start, $end)
   {
      ...
      if ($this->pun_config['o_indent_num_spaces'] != 8 && $start == '[code]')
      {
         $spaces = str_repeat(' ', $this->pun_config['o_indent_num_spaces']);
   ...
   function _handle_img_tag($url, $is_signature = false)
   {
      ...
      if ($is_signature && $this->pun_user['show_img_sig'] != '0')
      {
         $img_tag = '<img class="sigimage" src="'.$url.'" alt="'.htmlspecialchars($url).'" />';
      }
      else if ( ! $is_signature && $this->pun_user['show_img'] != '0')
      {
   ...
   function _do_bbcode($text)
   {
      if (strpos($text, 'quote') !== false)
      {
         ...
         $text = preg_replace('#\[quote=(&quot;|"|\'|)(.*)\\1\]#seU', '"</p><blockquote><div class=\"incqbox\"><h4>".str_replace(array(\'[\', \'\\"\'), array(\'&#91;\', \'"\'), \'$2\')." ".$this->lang_common[\'wrote\'].":</h4><p>"', $text);
   ...
   function _do_smilies($text)
   {
      ...
      for ($i = 0; $i < count($this->smiley_text); ++$i)
      {
         $text = preg_replace("#(?<=.\W|\W.|^\W)". preg_quote($this->smiley_text[$i], '#')."(?=.\W|\W.|\W$)#m", '$1<img src="img/smilies/'.$this->smiley_img[$i].'" width="15" height="15" alt="'.substr($this->smiley_img[$i], 0, strrpos($this->smiley_img[$i], '.')).'" />$2', $text);
      }
    ...
    * Parse message text
    */
   function _parse_message($text, $hide_smilies)
   {
...
//      }
      ...
      $text = $this->_pun_htmlspecialchars($text);
      ...
      if ($this->pun_config['o_make_links'] == '1')
      {
         $text = $this->_do_clickable($text);
      }

      if ($this->pun_config['o_smilies'] == '1' && $this->pun_user['show_smilies'] == '1' && $hide_smilies == '0')
      {
         $text = $this->_do_smilies($text);
      }

      if ($this->pun_config['p_message_bbcode'] == '1' && strpos($text, '[') !== false && strpos($text, ']') !== false)
      {
         $text = $this->_do_bbcode($text);

         if ($this->pun_config['p_message_img_tag'] == '1')
         {
      ...
      if (isset($inside))
      {
         ...
         for ($i = 0; $i < $num_tokens; ++$i)
         {
            ...
            if (isset($inside[$i]))
            {
               ...
               $text .= '</p><div class="codebox"><div class="incqbox"><h4>'.$this->lang_common['Code'].':</h4><div class="scrollbox" style="height: '.$height_str.'"><pre>'.$inside[$i].'</pre></div></div></div><p>';
            }
         }
      }
    ...
    */
   function _parse_signature($text)
   {
//      if ($this->pun_config['o_censoring'] == '1')
//      {
//         $text = censor_words($text);
//      }
      ...
      if ($this->pun_config['o_smilies_sig'] == '1' && $this->pun_user['show_smilies'] != '0')
      {
         $text = $this->_do_smilies($text);
      }

      if ($this->pun_config['p_sig_bbcode'] == '1' && strpos($text, '[') !== false && strpos($text, ']') !== false)
      {
         $text = $this->_do_bbcode($text);

         if ($this->pun_config['p_sig_img_tag'] == '1')
         {
    ...
    */
   function _pun_htmlspecialchars($str)
   {
