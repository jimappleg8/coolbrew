...
class Ad_ldap
{
    ...
    *
    */
   function Ad_ldap($options = array())
   {
      // you can specifically overide any of the default configuration 
      // options setup above
      if (count($options) > 0)
      {
      ...
      if ($this->_ad_username != NULL && $this->_ad_password != NULL)
      {
         ...
         if ( ! $this->_bind)
         {
            // if you have problems troubleshooting, remove the @ character
            // from the ldap_bind command above to get the actual error message
            if ($this->_use_ssl)
            {
            ...
            {
               echo ("FATAL: AD bind failed. Check the login credentials. Error: ".ldap_error($this->_conn));
               exit();
            }
         }
      }
   ...
   /**
    * closes the connection with active directory
    *
    */
...
/*
In practice the full unique user name as known to LDAP is a hierarchically 
qualified string, the user is unlikley to know this and the normal approach 
to authentication is a two stage process, the first stage uses an anonymous 
LDAP bind so that the fully qualified user name can be determined from the 
database, the second stage attempts a non-anonymous bind using the fully 
qualified user name determined in the first stage and a user supplied 
password.

The advantage of the two stage process is that it is simpler for the end 
user and is immune to any changes that system administrators may make to 
the LDAP hierarchy (it has happened !). It may be thought that requiring 
that the user already have a unique identification is a weakness. This 
is to some extent true, but it would also be possible to authenticate 
users based on department name and surname or e-mail address or any other 
combination of LDAP information, if more than one fully qualified user 
name is returned in response to the anonymous LDAP query, then it would 
be possible to try a non-anonymous bind for each user name in turn.

	$ldapserver = ldap_connect("ldapserver.wlv.ac.uk");
	if(!$ldapserver)
	{
	...
	$base_dn = "DC=unv,DC=wlv,DC=ac,DC=uk";
	$filter = "cn=" . $userid;
	$inforequired = array("employeeType",
		"department","employeeID","mail","givenName","sn");
...
anonymously so you have to bind with a (known) user and password. Or 
else you will get an Search operations error. I also can confirm that 
an empty password bind succeeds! So test for an empty password first!
*/
    ...
    *
    * adding a user is a bit fiddly, we need to get the full DN of the 
    * user and add it using the full DN of the group
    */
   ...
   /**
    * Returns AD users or groups based on search and fields
    *
    */
...
//      echo "<pre>"; print_r($entries); echo "</pre>";
   ...
   // ====================================================================
   //  UTILITY FUNCTIONS 
   //  not intended to be called directly but I spose you could?
   // ====================================================================

   /**
    * ldap doesn't like NULL attributes, only set them if they have values
    * 
    * I'd like to know how to set an LDAP attribute to NULL though, at the moment
    * I set it to a space
    *
    */
...
*/
    ...
    *
    * this is just a list of characters with known problems and I'm trying
    * not to strip out other languages
    *
    */
   function ldap_slashes($str)
   {
      ...
      // replace them
      $str = str_replace($illegal, $legal, $str);
   ...
   function account_control($options)
   {
      ...
      if (is_array($options))
      {
         ...
         // PASSWD_CANT_CHANGE Note You cannot assign this permission by directly 
         // modifying the UserAccountControl attribute. For information about how 
         // to set the permission programmatically, see the "Property flag 
   ...
   /**
    * Take an ldap query and return the nice names, without all 
    * the LDAP prefixes (eg. CN, DN)
    *
    */
