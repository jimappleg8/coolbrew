...
class Nleas extends Controller {
   ...
   /**
    * Copies an NLEA from another product
    *
    * Auditing: complete
    */
   function copy($site_id, $product_id, $this_action) 
   {
      ...
      $rules['SourceProduct'] = 'trim'; //|required';
      $rules['SourceProductID'] = 'trim'; //|required';

      $this->validation->set_rules($rules);

      $fields['SourceProduct'] = 'Source Product';
      $fields['SourceProductID'] = 'Source Product ID';
      ...
      if ($this->validation->run() == FALSE)
      {
         ...
         $data['products'] = $this->Nleas->get_product_list($site_id);
      
         $this->load->vars($data);
   	
         return $this->load->view('nleas/copy', NULL, TRUE);
      }
      else
      {
   ...
   /**
    * Copies an NLEA record from another product
    *
    * Auditing: complete
    */
   function _copy($site_id, $product_id) 
   {
      if ( ! $this->administrator->check_acl($site_id.'-site', 'view'))
         redirect('cp/login/sorry');
      ...
      // determine if a product ID was entered. If it was, it overrides
      // any selection from the pulldown menu.
      if ($values['SourceProductID'] != '')
      {
         $source_id = $values['SourceProductID'];
      }
      else
      {
         $source_id = $values['SourceProduct'];
      }

      if ($source_id == 0)
         return;

      $old_nlea = $this->Nleas->get_nlea_data($product_id);
      $nlea = $this->Nleas->get_nlea_data($source_id);
      unset($nlea['ProductID']);
      
      $this->Nleas->update_nlea($product_id, $old_nlea, $nlea);

      $this->session->set_userdata('product_message', 'The nutrition facts have been copied.');
   ...
   function edit($site_id, $product_id, $this_action) 
   {
      ...
      $fields['OverrideHTML'] = 'HTML to override other settings';
      ...
      if ($this->validation->run() == FALSE)
      {
         $this->collector->append_css_file('admin');
         $this->collector->append_css_file('products');
         $this->collector->append_css_file('products-tags');
...
//                               '7' => 'CA Baby Food (French)',
         ...
         $data['site'] = $site;
         $data['product'] = $this->Products->get_product_data($product_id, $site_id);
         $data['nutrition_facts'] = $this->Products->nutrition_facts($product_id);
