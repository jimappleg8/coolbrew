...
//      $perpage = $result['RESULTS']['QUERY']['STORESPERPAGE'];
      ...
      if ($this->total_stores > 0)
      {
         ...
         for ($i=0; $i<count($xml_stores); $i++)
         {
            ...
            $stores[$cnt]['Phone'] = $this->format_phone($xml_stores[$i]['phone']);
    ...
    * The format seems to have dropped the dash in recent times.
    * 
    */
   function format_phone($phone)
   {
      return preg_replace('/(\(\d\d\d\)) (\d\d\d)(\d\d\d\d)/', '$1 $2-$3', $phone);
   }
...
/* Location: ./system/modules/stores/models/nielsen_model.php */